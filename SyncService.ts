...
const {TIME,BLOCKCOUNT, NONE} = FabricConst.PurgeModes;
...
export class SyncServices {
	...
	async insertNewChannelChaincode(
		...
		// console.log("%%%%%%%%%%%%%%% CLIENT FABRICGATEWAY%%%%%%%%%%%",client.fabricGateway)
		const chaincodes = await client.fabricGateway.queryInstantiatedChaincodes(
			channel_name
		);
		// console.log("%%%%%%%%%%%% CHAINCODE BY CHANNEL %%%%%%%%%%%", chaincodes);
	...
	async syncBlocks(client, channel_name, noDiscovery) {
		//changes from PR#341 - Added by Anusha
		...
		// console.log("<<<<<<<Missing blocks>>>>>>>>>", results);
		...
		if (results) {
			for (const result of results) {
				...
				try {
					...
					if (block) {
						// condition to put purgge mode NONE
						//Anusha
						//Changes for Purge
						//Udhaya & Manish
						// if (this.validatePurgemode(purgeMode) {
						if (this.validatePurgemode(purgeMode) && purgeMode != "NONE") {
							const lastBlockTo = await this.persistence
								.getCrudService()
								.fetchLastBlockToFromExplorerAudit(
									purgeMode,
									channel_genesis_hash,
									network_id
								);
							if (result.missing_id > lastBlockTo) {
								// console.log("processBlockEvent 1=============",block.data.data[0].payload.action.proposal_response_payload.extension);
								await this.processBlockEvent(client, block, noDiscovery);
							} else {
								logger.info(
									`Not saving this block to DB since we are keeping only recent data # ${result.missing_id}`
								);
							}
						} else {
							await this.processBlockEvent(client, block, noDiscovery);
							// console.log("processBlockEvent 2=============",block.data.data[0].payload.action.proposal_response_payload.extension);
	...
	async processBlockEvent(client, block, noDiscovery) {
		...
		// console.log('=======first_tx======', first_tx);
		
		// The 'header' object contains metadata of the transaction
		// console.log("---------processBlockEvent main----------")
		...
		//Anusha
		// console.log("New Block  >>>>>>> %j", block.header.number);
		//Anusha
		const channelInfo = await client.fabricGateway.queryChainInfo(channel_name);
		// console.log("----------------channelInfo------------ ", channelInfo);
		// console.log("------------------channel_name------------------- ", channel_name);

		const maxHeight = parseInt(channelInfo.height.low);
		...
		// console.log("@@@@@@@@@@@ inside block_row @@@@@", block_row);
		...
		for (let txIndex = 0; txIndex < txLen; txIndex++) {
			...
			// console.log("txObj=========debug=======",txObj.payload.data.actions[0].payload.action.proposal_response_payload.extension);
			// console.log("====txObj====txIndex=====", txObj, txIndex);
			...
			if (txid && txid !== "") {
				...
				// console.log("validation_codes", validation_codes);
			...
			if (txObj.payload.data.actions !== undefined) {
				...
				// console.log("LOgging Chaincode ::::", chaincode);
				// Saundarya Start
				chaincodeversion =
					txObj.payload.data.actions[0].payload.action.proposal_response_payload
						.extension.chaincode_id.version;
				// console.log(
				// 	"Logging Chaincode version In Transaction Object ::::",
				// 	chaincodeversion
				// );
				// Saundarya Ends
				...
				endorser_id_bytes =
					txObj.payload.data.actions[0].payload.action.endorsements[0].endorser
						.IdBytes;
						// console.log('>>>>>>>>>>>>ENDORSER ID BYTES<<<<<<<<<<<<<<<<', endorser_id_bytes);
			...
			// console.log("<<<<<TX ROW>>>>>", transaction_row);

			// Insert transaction
			//Anusha
			// console.log("==========================debug for its run or not ===================")
			const res = await this.persistence
				.getCrudService()
				.saveTransaction(network_id, transaction_row, maxHeight);
			logger.debug("saveTransaction ", res);
			// console.log('======save transaction=====', res);
		...
		const successSaveBlock = await this.persistence
			.getCrudService()
			.saveBlock(network_id, block_row, maxHeight);
		logger.debug("result of SaveBlock ", successSaveBlock);
		// console.log("<<<<<<Inside Insert Block>>>>>>", block_row);
		//Anusha

		// Saurabh
		// Changes for Add Block To Channel

		// console.log("======result of SaveBlock======= ", successSaveBlock);
		if (successSaveBlock) {
			// Push last block
			const notify = {
	...
	validatePurgemode(purgeMode: string) {
		if (Object.keys(purge_mode).includes(purgeMode)) {
			return true;
		} else return false;
	}
	// Saundarya Ends
...
// Transaction validation code
// Anusha added export in this to re use in proxy.ts
